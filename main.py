...
def main():
    ...
    with tempfile.TemporaryDirectory() as temp_path:
        try:
            ...
            for message_idx in tqdm(range(nof_messages)):
                ...
                if mail.is_valid_message(message):
                    ...
                    print(
                        f"number {k}: is Valid   the mail is: {filtered_mail_list[0] if len(filtered_mail_list) > 0 else ''}")
                    pass
                else:
                    print(f"number {k}: is not Valid.")
                k = k + 1
                mail.untag_message(message)
